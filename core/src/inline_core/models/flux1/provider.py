"""FLUX.1's answer to "what do I need on disk" - the model popup's data source for the node."""
...
class Flux1Provider:
    """Requirements + fit estimate for the FLUX.1 node.

    One provider covers every build: dev, schnell, Kontext, Krea and the Fill/Control checkpoints
    share the same VAE and the same two encoders, so only the transformer row changes.
    """

    def components(self, params: dict[str, object] | None = None) -> list[ModelComponent]:
        return flux1_requirements(params)
    ...
    def catalog_options(self, category: str) -> list[str] | None:
        """Only the files this node can actually load. The categories are shared with Z-Image,
        Krea 2 and FLUX.2, so an unfiltered list offers checkpoints that would fail on load."""
        if category == "diffusion_models":
            return [p.name for p in flux1_checkpoints()]
        if category == "text_encoders":
            return [p.name for p in flux1_encoders()]
        return None
